...
int main() {
   ...
   * Reference:
   * https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html#ga37d3796527d4c7711d99265180cabaae
   */
  float *d_A, *d_B, *d_C;
  // cudaMalloc(&d_A, ...);
  // ...
   ...
   * Reference:
   * https://docs.nvidia.com/cuda/cuda-runtime-api/group__CUDART__MEMORY.html#ga48efa57b3ca9623e595c25605d398d83
   */
   ...
   * Use a 1D grid with 256 threads per block. Calculate the number of blocks
   * needed. Reference:
   * https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#kernels
   */
  ...
  for (int i = 0; i < N; i++) {
    if (std::fabs(h_C[i] - (h_A[i] + h_B[i])) > 1e-5) {
      ...
      std::cerr << "Expected: " << h_A[i] + h_B[i] << ", Got: " << h_C[i]
                << std::endl;
      success = false;
      break;
    }
